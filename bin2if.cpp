...
int main (int argc, char ** argv) {
	...
	if(strcmp(observatory, "gbt") == 0) {
		clock               = 3000;	     	// an instrument configurable
		cc_per_sys	    = 4096;
		cc_per_instance     = 512;
		start_cc	    = 0;		// an instrument configurable
    	fc_per_cc           = pow(2.0, 19);    		// 512K, 19 bits, 
							//  all bits 1 = 0x0007FFFF, high bit = 0x00040000
	} else if (strcmp(observatory, "ao") == 0) {
		clock               = 896;	       	// an instrument configurable
		cc_per_sys	        = 4096;
		cc_per_instance     = 320;
		start_cc			= 1006;	// an instrument configurable
